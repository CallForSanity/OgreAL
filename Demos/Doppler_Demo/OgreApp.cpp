...
void OgreApp::createScene()
{
	...
	node = sceneMgr->getRootSceneNode()->createChildSceneNode("CarNode");
	node = node->createChildSceneNode(Vector3(50, 0, 0));
	ent = sceneMgr->createEntity("Civic", "Civic.mesh");
	node->attachObject(ent);

	OgreAL::Sound *sound = soundManager->createSound("BusSound", "motor_b8.wav", true);
