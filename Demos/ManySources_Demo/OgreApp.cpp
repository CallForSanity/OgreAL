...
void OgreApp::createScene()
{
	...
	int numSources = soundManager->maxSources();
	std::stringstream ss;
	int x = -200 * numSources;
	int z = -50;
	for(int i = 0; i < numSources * 2; i++)
	{
		ss.clear(); ss.str("");
		ss << "Cube" << i;
		node = sceneMgr->getRootSceneNode()->createChildSceneNode(ss.str());
		ent = sceneMgr->createEntity(ss.str(), "cube.mesh");
		node->attachObject(ent);
		node->setScale(Ogre::Vector3(0.25));
		node->setPosition(x, 50, z);
		x += 200;

		ss.clear(); ss.str("");
		ss << "Zero Factor" << i;
		sound = soundManager->createSound(ss.str(), "Zero Factor - Decay.ogg", true);
		sound->setGain(0.5);
